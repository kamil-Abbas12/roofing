...
import { Clock, MapPin, PhoneCall, X } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
        ...
        <h2 id="consult-modal-heading" className="text-xl font-bold text-blue-900 mb-1">
          Speak With a Specialist
        </h2>
        <p className="text-sm text-gray-500 mb-5">
          Tell us what you need and we&apos;ll connect you right now.
        </p>
        <CampaignCallFlow source="consult_button" onDone={onClose} />
      </div>
    </div>
  );
}
      ...
      <div className="w-full flex flex-col lg:flex-row justify-between items-stretch gap-8 mt-10 lg:mt-20">
        ...
        <div className="w-full lg:w-1/2 bg-white shadow-xl rounded-2xl p-6 md:p-8 hover:shadow-2xl transition-all duration-300 animate-slideUp">
          <div className="mb-6">
            ...
            <span className="text-xl md:text-2xl font-bold text-blue-800 ml-2">contact form or</span>
            <button
              type="button"
              onClick={() => setShowConsultModal(true)}
              className="text-xl md:text-2xl font-bold text-blue-900 ml-2 underline decoration-2 underline-offset-2 hover:text-blue-700"
            >
              call us now
            </button>
          </div>
        ...
        <div className="w-full lg:w-1/2 flex flex-col gap-4">
          <button
            ...
            aria-label="Talk to a roofing marketing specialist now"
          >
          ...
          <div className="bg-white shadow-lg rounded-xl p-8 hover:shadow-2xl transition lg:hover:scale-105">
            ...
            <address className="not-italic flex flex-col gap-8 text-gray-700">
              ...
              <button
                type="button"
                onClick={() => setShowConsultModal(true)}
                className="flex items-center gap-5 min-h-[40px] text-left"
              >
                <PhoneCall size={24} className="text-blue-900 shrink-0" />
                <span className="text-[18px]">Call a specialist now</span>
              </button>
